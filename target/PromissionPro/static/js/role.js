$(function () {
    ...
    $("#role_data1").datagrid({
        ...
        onClickRow:function (rowIndex,rowData) {//点击时候 触发
            ...
            for(var i=0;i<rows.length;i++){
                var row=rows[i];
                //如果已选权限id 等于 你所有权限其中的id 让其选中状态
                // console.log(rowIndex) 下标从0开始 数据的pid从1开始
                // console.log(rowData.pid)
        ...
        if(!rowData){
            ...
            $.messager.confirm("提示","是否做删除操作",function (res) {
                if(res){
                    ...
                    $.get("deleteRoleByRid"+rowData.rid,function (data) {
                        if(data.success){
